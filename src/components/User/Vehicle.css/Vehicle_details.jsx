...
import { useUser } from "@clerk/clerk-react" // Ensure this is imported
import BookingForm from "../../Booking/BookingForm" // Assuming this component exists
import './vehicle-details.css'

const cloudinaryBase = "https://res.cloudinary.com/duortzwqq/image/upload"

export default function VehicleDetails() {
  const { vehicleId } = useParams() // Changed from 'id' to 'vehicleId' to match route
  ...
  const [ratingSubmitMessage, setRatingSubmitMessage] = useState(null)
  const [ratingMessageType, setRatingMessageType] = useState("") // 'success' or 'error'

  const { user, isLoaded: isUserLoaded } = useUser()

  // Function to fetch a single vehicle's data
  const fetchVehicle = async (idToFetch) => {
    console.log(`[VehicleDetails] Fetching vehicle with ID: ${idToFetch}`)
    setIsLoadingVehicle(true)
    try {
      const vehicleRef = doc(db, "vehicles", idToFetch)
      const vehicleSnap = await getDoc(vehicleRef, { source: "server" }) // Explicitly request from server
      if (vehicleSnap.exists()) {
        ...
        const processedVehicle = {
          ...
          // reviewsData will be fetched separately by fetchVehicleRatings
        }
        setVehicle(processedVehicle)
        console.log("[VehicleDetails] Processed vehicle state set:", processedVehicle)
      } else {
        console.log(`[VehicleDetails] Vehicle with ID ${idToFetch} not found.`)
        setVehicle(null)
      }
    } catch (err) {
  ...
  const fetchVehicleRatings = async (idToFetch) => {
    ...
    try {
      ...
      fetchedRatings.sort((a, b) => b.createdAt - a.createdAt) // Sort by most recent
      ...
      setRatingSubmitMessage("Failed to load reviews.")
      setRatingMessageType("error")
    }
  }

  // Effect to fetch vehicle details when vehicleId changes
  useEffect(() => {
    if (vehicleId) {
  ...
  // Effect to fetch ratings when activeTab is 'reviews' or vehicle data changes
  useEffect(() => {
    if (activeTab === "reviews" && vehicleId) {
      fetchVehicleRatings(vehicleId)
    }
  }, [activeTab, vehicleId]) // Depend on activeTab and vehicleId

  // Handle modal open/close for booking form
  useEffect(() => {
    if (showBookingForm) {
  ...
  // Handler to fix the "BOOKED" problem (re-fetch vehicle status after booking)
  const handleCloseBookingForm = (bookedSuccessfully) => {
    ...
    if (bookedSuccessfully) {
      ...
      fetchVehicle(vehicleId) // Re-fetch the vehicle data to update 'isAvailable'
    }
  }
  ...
  const handleSubmitRating = async (e) => {
    ...
    if (!isUserLoaded || !user) {
      setRatingSubmitMessage("Please log in to submit a review.")
      setRatingMessageType("error")
      return
    }
    if (newRating === 0) {
      setRatingSubmitMessage("Please select a star rating.")
      setRatingMessageType("error")
      return
    }
    if (newComment.trim() === "") {
      setRatingSubmitMessage("Please write a comment for your review.")
      setRatingMessageType("error")
      return
    }

    setIsSubmittingRating(true)
    setRatingSubmitMessage(null)

    try {
      await addDoc(collection(db, "ratings"), {
        vehicleId: vehicle.id,
        userId: user.id,
        userEmail: user.primaryEmailAddress?.emailAddress || user.emailAddresses[0]?.emailAddress || "N/A",
        userName: user.username || `${user.firstName || ""} ${user.lastName || ""}`.trim() || "Anonymous",
        userImageUrl: user.imageUrl || "/placeholder.svg?height=50&width=50&text=User", // Include user image URL
        rating: newRating,
        comment: newComment.trim(),
        createdAt: serverTimestamp(),
      })
      setRatingSubmitMessage("Review submitted successfully!")
      setRatingMessageType("success")
      setNewRating(0)
      setNewComment("")
      // Re-fetch ratings to update the list and average
      fetchVehicleRatings(vehicle.id)
    } catch (error) {
      console.error("Error adding document: ", error)
      setRatingSubmitMessage("Failed to submit review. Please try again.")
      setRatingMessageType("error")
    } finally {
      ...
      <div className="container">
        ...
        <div className="vehicle-header">
          <div className="vehicle-title-section">
            <h1 className="vehicle-title">{vehicle.name || `${vehicle.make} ${vehicle.model}`}</h1>
                    ...
                    {isUserLoaded && user ? (
                      <form onSubmit={handleSubmitRating} className="rating-form">
                        <div className="form-group">
                          <label htmlFor="star-rating-input" className="form-label">
                            Your Rating
                          </label>
                          <div className="star-rating-input" id="star-rating-input">
                            {[1, 2, 3, 4, 5].map((starValue) => (
                              <Star
                                key={starValue}
                                size={28}
                                className={`star-icon ${starValue <= newRating ? "filled" : ""}`}
                        ...
                        <button type="submit" className="submit-btn" disabled={isSubmittingRating}>
                          {isSubmittingRating ? "Submitting..." : "Submit Review"}
                        </button>

                        {ratingSubmitMessage && (
                          <p className={`submit-message ${ratingMessageType === "success" ? "success" : "error"}`}>
                            {ratingSubmitMessage}
                          </p>
                        )}
                      </form>
                    ) : (
                      <p className="login-to-review-message">Please log in to leave a review.</p>
                    )}
                  </div>
